...
try:
    #Get Access file ###
    con_string = r'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=https://api.render.com/deploy/srv-ceq833arrk0ah0mlt61g?key=jQTddIspSSw/opt/render/project/src/La Familia DB.accdb;'
    #File Connection
...
##-----------------------------GetUserInfo -----------------------------##
    ...
    def GetUserInfo(target:str,type:str): 
        if (target == '*'):
            cur.execute('SELECT * FROM Users')   
        else:
            if (type == 'NUM'):
                cur.execute('SELECT * FROM Users WHERE UserID = ?',(target))   
            else:
                cur.execute('SELECT * FROM Users WHERE FirstName AND LastName = ?',(target.split(' ')))   
